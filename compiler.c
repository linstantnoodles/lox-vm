...
// distinguish to plevel code vs body of function
typedef enum {
  TYPE_FUNCTION,
  TYPE_SCRIPT
} FunctionType;

// q: feels weird to name this struct "compiler"??
...
static void patchJump(int offset) {
  ...
  // the offset is the IP right before the emit jump added the offset
  // values. so say the current count is 10 and the emit jump position
  // is 7. then the jump needs to be at least 3 since the difference
...
static void declareVariable() {
  ...
  // basically looks for re-definitions in the SAME
  // scope level
...
static uint8_t parseVariable(const char* errorMessage) {
  ...
  // so really this only deals with globals
...
static void namedVariable(Token name, bool canAssign) {
  // more complex lookup since it can either be global or local
  // now
  uint8_t getOp, setOp;
  // an index that gets returned
...
static void block() {
  // as long as we haven't reach the closing brace
  // and are not EOF..
...
ObjFunction* compile(const char* source) {
  ...
  //   // if it doesnt equal to current line
  //   // print the line number along with value
  //   if (token.line != line) {
